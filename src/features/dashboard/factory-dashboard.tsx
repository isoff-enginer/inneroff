...
import { useFactoryDashboardData } from "@/features/dashboard/use-dashboard-data";
import { formatCurrency, greeting } from "@/lib/format";

export function FactoryDashboard() {
  const { user } = useSession();
  const { inventory, isLoading } = useFactoryDashboardData(user?.factoryId);

  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Cargando métricas de fábrica...</div>;
  }

  const stats = [
    { label: "Inventario Local", value: formatCurrency(inventory), hint: "valorizado" },
    { label: "Despachos", value: "0", hint: "en tránsito" },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        description="Panel de operación de fábrica."
      />
      ...
      <div className="mt-8 surface p-8 text-center text-muted-foreground border-t border-border">
        ...
        <p className="mt-2 text-sm">El historial de producción y destinos se conectarán en la siguiente actualización.</p>
      </div>
    </>
  );
}
